...
def get_last_frame(image_dir):
    image_files = glob.glob(os.path.join(image_dir, "frame_*.jpg"))
    if not image_files:
        return None

    # Sort images by numeric x value in frame_xxxx.jpg
    image_files.sort(key=lambda x: int(os.path.basename(x).split("_")[-1].split(".")[0]), reverse=True)

    return image_files[0]  # Return the highest-numbered frame
...
for x_dir in os.listdir(root_dir):
    ...
    video_dir = os.path.join(x_path, "task/extracted_frames_Video")
...
def show_image(task):
    ...
    image = cv2.imread(image_path)  # Read image directly with cv2
    if image is None:
