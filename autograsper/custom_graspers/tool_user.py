...
from object_tracker.ShapeAwarePoseEstimator import ShapeAwarePoseEstimator
...
class ToolUser(AutograsperBase):
    ...
    def check_grasping_success(self):
        ...
        object_position = find_tool(
            self.bottom_image,
            comparison_source= self.config["tool_outline"],
            mm_to_pixels= self.config["mm_to_pixels"],
    ...
    def compute_caging_metric(self, object_position):
        state = self.robot_state

        

    def check_collection_success(self):
        state = self.robot_state
        ...
        object_position = get_object_pos(
            self.bottom_image,
            self.robot_idx,
            color= self.config["object_color"],
        )

        if object_position is None:
            return False
        
        object_collected = self.compute_caging_metric(object_position)> self.config["collection_criterion"]
...
def find_tool(image, comparison_source, mm_to_pixels, camera_matrix, dist_coeffs, output_path=None):
    """
    Find the tool in the image using the ShapeAwarePoseEstimator.
    
    Parameters:
    -----------
    image : numpy.ndarray
        OpenCV image object (BGR format)
    comparison_source : str or list
        Path to the object outline file or a list of points
    mm_to_pixels : float
        Scaling factor to convert mm to pixels
    ...
    --------
    tuple
        (center_coordinates, output_image)
        center_coordinates: (x, y) coordinates or None if no object found
        output_image: image with marked object center
    """
    # Initialize the pose estimator with the object outline and color range
    if isinstance(comparison_source, str):
        # Load the object outline from a file
        comparison_source = cv2.imread(comparison_source, cv2.IMREAD_GRAYSCALE)
        if comparison_source is None:
            raise ValueError(f"Could not load image from {comparison_source}")
    elif isinstance(comparison_source, list):
        # Convert list of points to a numpy array
        comparison_source = np.array(comparison_source, dtype=np.float32)
    else:
        raise ValueError("comparison_source must be a file path or a list of points")
    
    pose_estimator = ShapeAwarePoseEstimator(
        comparison_source,
        color_range_low=(0, 0, 0),
        color_range_high=(180, 255, 255),
        mm_to_pixel=mm_to_pixels,
        camera_matrix=camera_matrix,
        dist_coeffs=dist_coeffs,
    )
    
    # Find the object in the image
    pose, tool_outline = pose_estimator.estimate_pose(image)
    if pose is None:
    ...
    # Optionally draw the contour
    cv2.drawContours(output_image, tool_outline, 0, (0, 255, 0), 2)
    # Save the output image if path provided
    if output_path is not None:
        cv2.imwrite(output_path, output_image)
        print(f"Image with marked object saved to {output_path}")
    
    return [x,y],angle,output_image
