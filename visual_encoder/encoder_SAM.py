...
from ImageDatasetSeg import ImageDataset as ID
import cv2
import matplotlib
import json
from ultralytics import FastSAM as SAM
import logging
from ultralytics.utils import LOGGER

LOGGER.setLevel(logging.ERROR)  # Suppress most logs
...
class CNNFeatureExtractor(nn.Module):
    ...
    def __init__(self, output_dim=256):
        ...
        self.conv = nn.Conv2d(1280, output_dim, kernel_size=5, stride=4, padding=1)
        # self.fc = nn.Linear(256 * 3 * 2, output_dim)  # Flattened CNN output
    def forward(self, x):
...
class MaskFeatureExtractor(nn.Module):
    def __init__(self, output_dim=128):
        ...
        self.cnn = nn.Sequential(
            nn.Conv2d(6, 16, kernel_size=3, stride=1, padding=1),  # Output: (16, H, W)
            nn.ReLU(),
            nn.MaxPool2d(2, 2),  # Downsample

            nn.Conv2d(16, 32, kernel_size=3, stride=1, padding=1),  # (32, H/2, W/2)
            nn.ReLU(),
            nn.MaxPool2d(2, 2),  # Downsample

            nn.Conv2d(32, 64, kernel_size=3, stride=1, padding=1),  # (64, H/4, W/4)
            nn.ReLU(),
            nn.AdaptiveAvgPool2d((4, 4))  # Fixed-size output (64, 4, 4)
        )
        
        # self.fc = nn.Linear(64 * 4 * 4, output_dim)  # Flattened CNN output
...
class HybridCNNTransformer(nn.Module):
    ...
    def __init__(self, feature_dim=256):
        super().__init__()
        self.cnn_mask = MaskFeatureExtractor(output_dim=feature_dim)
        self.cnn_image = CNNFeatureExtractor(output_dim=feature_dim)
        self.transformer = TransformerEncoder(feature_dim=2*feature_dim)
        # self.segmentation_head = SegmentationHead(input_dim=feature_dim, output_channels=1)
        self.regression_head = PositionRegressionHead(input_dim=2*feature_dim, output_dim=5)

    def forward(self, image,mask):
        mask_features = self.cnn_mask(mask)  # CNN feature extraction
        image_features = self.cnn_image(image)  # CNN feature extraction
...
SAM_model = SAM('FastSAM-s.pt')
SAM_model.eval()  # Set to evaluation mode
# Freeze SAM
for param in SAM_model.parameters():
    param.requires_grad = False
SAM_model.to("cpu")  # Move to CPU
# Instantiate model
...
mask_transform = transforms.Compose([
    transforms.Resize((128, 128), antialias=True),  # Resize tensor masks
    transforms.Lambda(lambda mask: mask.float())  # Ensure correct dtype
])
...
# Load dataset
dataset = ID(sequence_dirs, transform=image_transform, mask_transform=mask_transform,preproc_model=SAM_model)# if no split is needed
...
for epoch in range(num_epochs):
    ...
    for images, masks, metadata in train_loader:
        images, masks, metadata = images.to(device), masks.to(device), metadata.to(device)  # Move batch to GPU
    ...
    if epoch % 10 == 0:
        ...
        torch.save(model.state_dict(), f"yolo_model_checkpoint_{epoch}.pth")
...
# Save loss history
with open("yolo_training_losses.json", "w") as f:
...
with open("yolo_dataset_splits.pkl", "wb") as f:
