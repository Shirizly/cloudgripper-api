...
class curved_tool:
    ...
    def compute_curve_outline(self, step=0.01, eps=0.2):
        ...
        curve_points = []
        for i,seg in enumerate(self.curve_parameters):           
            if seg[0] == 'line':
                # seg: ('line', p1, p2)
                _, p1, p2 = seg
                t_vals = np.arange(0, 1 + step, step)
        ...
        right_offsets = self.modify_curve_outline(right_offsets,left_offsets)
    ...
    def modify_curve_outline(self,right_offsets,left_offsets):
        """
        modify the last 20 mm of the outline to include a special detail, where the right side offset tapers to a point."""
        # find the point where the last 20 mm of the offsets starts
        last_segment_length = 50 # total length is 40, 20 need to be modified, 100 points per segment
        last_segment_start = len(right_offsets) - last_segment_length
        t = np.linspace(0, 1, last_segment_length)
        right_offsets[last_segment_start:] = (np.array([1-t,1-t]).T*right_offsets[last_segment_start:] + np.array([t,t]).T*left_offsets[last_segment_start:])
        return right_offsets
...
if __name__ == "__main__":
    ...
    outline = tool.compute_curve_outline(step=0.01,eps = 2.5)
    np.savetxt('object_outline.txt', outline)
